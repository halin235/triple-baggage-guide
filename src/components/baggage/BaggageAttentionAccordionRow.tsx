...
import type { BaggageCardTone, BaggageRegulationItem } from "@/types/baggage";

const TRIPLE_BLUE = "#3182F6";
const POINT_RED = "#FF4B4B";

/** 엑셀 `카드 컬러` → 좌측 보더·아이콘 톤 (RED / ORANGE / BLUE) */
...
/**
 * 엑셀 필드: 소분류(품목명), 카드 컬러(보더/아이콘), 안내 문구 요약(배지), 안내 문구 상세(펼침)
 */
export function BaggageAttentionAccordionRow({
  row,
  open,
  onToggle,
}: {
  ...
  const { visual } = getBaggageBadge(row);
  const barColor = accentFromCardTone(row.cardTone);
